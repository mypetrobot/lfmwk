...
class PlaysUnderCommand extends Command {

    constructor() {
        super({
            name: 'playsunder',
            description: 'Displays the total number of artists with less than the specified number of plays by a given user over a specified time period. The results are limited to the user\'s top 1000 artists for that time period. If no user is specified, the bot will default to you. If no time period is specified, the bot will default to overall.',
            usage: [
				'playsunder <threshold>',
				'playsunder <threshold> <time period w/m/q/y/o>',
				'playsunder <threshold> <user>',
				'playsunder <threshold> <time period w/m/q/y/o> <user>'
				],
			aliases: ['pu']
        })
	}
    ...
    async run(client, message, args) {
			...
			if(isNaN(pthresh)){
				await message.reply('you need to specify a playcount threshold.')
				return
			}
			...
			let msg='**'+elig+'** of **'+user.username+'**\'s top **'+dataartist.topartists.artist.length+'** '+plur("artist",dataartist.topartists.artist.length)+' '+timeperiod+' have less than **'+pthresh+"** "+plur("play",pthresh)
...
module.exports = PlaysUnderCommand