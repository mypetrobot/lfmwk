        ...
        if (data.error) {
            ...
            if (artist[`@attr`] && artist[`@attr`].nowplaying) {
				...
				try{
					...
					artistName=data3.artist.name
					let pretag=[]
					let badtags = [
						'seen live'
					]
					badtags.push(artistName.replace("-", " ").toLowerCase())
					for(let i=0;i<data3.artist.tags.tag.length;i++){
						if(data3.artist.tags.tag[i].name.length<20){
							const matches = data3.artist.tags.tag[i].name.match(/\d+/g);
							if (matches != null) {							
							}else{
								pretag.push(data3.artist.tags.tag[i].name.replace("-", " ").toLowerCase())
							}
						}
					}
				...
				let info=[]
				if(userplaycount=="?"){
					info.push("No data for "+artistName+" on last.fm")
				}else{
					if(userplaycount!=1){
						info.push(""+userplaycount+" "+artistName+" scrobbles")
					}else{
						info.push(""+userplaycount+" "+artistName+" scrobble")
					}
				}
				...
				if (allcrowns.length > 0) {
				   //info.push('**'+ message.guild.members.cache.get(allcrowns[0].userID).user.username+'** has the crown for **' + artistName+'** with **'+ allcrowns[0].artistPlays + "")
					...
					try{
						if((artistName.indexOf('(')==-1)&&(artistName.indexOf(')')==-1)&&(data3.artist.url.indexOf('(')==-1)&&(data3.artist.url.indexOf(')')==-1)){
							msg="By **["+artistName+"]("+data3.artist.url+")**"							
						}else{
							msg="By **"+artistName+"**"
						}
					}catch{}
