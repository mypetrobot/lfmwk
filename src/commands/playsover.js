...
class PlaysOverCommand extends Command {

    constructor() {
        super({
            name: 'playsover',
            description: 'Displays the total number of artists with at least the specified number of plays by a given user over a specified time period. The results are limited to the user\'s top 1000 artists for that time period. If no user is specified, the bot will default to you. If no time period is specified, the bot will default to overall.',
            usage: [
				'playsover <threshold>',
				'playsover <threshold> <time period w/m/q/y/o>',
				'playsover <threshold> <user>',
				'playsover <threshold> <time period w/m/q/y/o> <user>'
				],
			aliases: ['po']
        })
	}
    ...
    async run(client, message, args) {
			...
			let msg='**'+elig+'** of **'+user.username+'**\'s top **'+dataartist.topartists.artist.length+'** '+plur("artist",dataartist.topartists.artist.length)+' '+timeperiod+' have **'+pthresh+"** or more "+plur("play",pthresh)
...
module.exports = PlaysOverCommand