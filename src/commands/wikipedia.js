...
class WikipediaCommand extends Command {
    ...
    constructor() {
        super({
            name: 'wikipedia',
            description: 'Displays a link to Wikipedia search results for a specified artist/track. ' +
            ...
            usage: [
                'wikipedia',
				'wikipedia <artist name>',
                'wikipedia <artist name> | <track name>'
            ],
            aliases: ['wiki']
        })
    }
    ...
    async run(client, message, args) {
        ...
        if (artistName.length > 0) {
				...
				const url=(`https://en.wikipedia.org/w/index.php?${urlp}`)
				//await message.channel.send(`Album cover for \`${artistName} - ${albumName}\`: ${url}`)
					...
					const embed = new BotEmbed(message)
						...
						.setURL(url)
						.setAuthor('\🔎 Wikipedia search for "'+tdisp+'"')
						.setThumbnail(cover)
