...
class TrackPercentCommand extends Command {

    constructor() {
        super({
            name: 'trackpercent',
            description: 'Shows the percentage of your total playcount for an artist that a specified track occupies. ' +
            'If no artist/track are specified, the bot will try to look up the track you are ' +
            'currently listening to.',
            usage: [
                'trackpercent', 
                'trackpercent <artist name> | <track name>',
				'trackpercent | @<username>', 
                'trackpercent <artist name> | <track name> | @<username>'				
            ],
            aliases: ['tpct']
        })
    }
    ...
    async run(client, message, args) {
        ...
        if (artistName.length > 0) {
			...
			userartistplaycount=parseInt(dataartist.artist.stats.userplaycount)
			
			const pct=Math.round(((userplaycount/userartistplaycount)*100)*100)/100
			
				let msg
				if(userplaycount>0){
					if(args.join(` `).split('|').length>1){
						//msg=user.username+" has scrobbled **"+artistName+"** **"+userplaycount+"** time"
						msg=user.username+"'s **"+userplaycount+"** scrobble"
						if(userplaycount>1){
							msg+="s"
						}
						msg+=" of **"+trackName+"** is equal to **"+pct+"%** of their total scrobbles of **"+artistName+"**."
					}else{
						msg="your **"+userplaycount+"** scrobble"
						if(userplaycount>1){
							msg+="s"
						}
						msg+=" of **"+trackName+"** is equal to **"+pct+"%** of your total scrobbles of **"+artistName+"**."
					}
				}else{
					if(args.join(` `).split('|').length>1){
...
module.exports = TrackPercentCommand