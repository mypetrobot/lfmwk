...
const pgvar = new Client({
  connectionString: process.env.DATABASE_URL,
  ssl: true,
});
/*
const pgurl=process.env.DATABASE_URL
const dbname=pgurl.split('/')[3]
const dbport=parseInt(pgurl.split(':')[3].split('/')[0])
const dbhost=pgurl.split(':')[2].split('@')[1]
const dbpw=pgurl.split(':')[2].split('@')[0]
const dbuser=pgurl.split(':')[1].split('/')[2]
*/
